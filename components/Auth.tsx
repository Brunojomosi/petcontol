...
import { PawPrint, Loader2, KeyRound, Mail, ArrowLeft, CheckCircle2 } from 'lucide-react';
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'signUp') {
        ...
        if (error) throw error;
        setMessage({ text: 'Senha atualizada com sucesso! Você já pode entrar.', type: 'success' });
      ...
      <div className="bg-white p-8 rounded-[2.5rem] shadow-xl shadow-gray-200/50 w-full max-w-md border border-gray-100 animate-fade-in">
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-gray-400 font-medium text-center mt-1">
            {mode === 'forgotPassword' ? 'Recupere seu acesso' : 
             mode === 'updatePassword' ? 'Defina sua nova senha' : 
             'Gerencie seu universo pet'}
          </p>
        </div>
